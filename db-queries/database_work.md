...
Вывод: 
```
В базе найдено 4 курьера, у которых есть заказы в статусе «В доставке». 
Лидер — courier_5 с 8 заказами.Скриншот результата: screenshots/query1_result.png
```
...
Вывод: 
```
В базе 20 записей заказов. Статусы распределены так:  1 («В доставке») — большинство  
